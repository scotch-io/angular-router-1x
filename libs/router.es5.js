...
function ngOutletDirective($animate, $injector, $q, $router) {
  ...
  function viewportLink(scope, $element, attrs, ctrls, $transclude) {
    var viewportName = attrs.ngOutlet || 'default',
        parentCtrl = ctrls[0],
        myCtrl = ctrls[1],
...
ngOutletDirective.$inject = ["$animate", "$injector", "$q", "$router"];
...
angular.module('ngComponentRouter').factory('$$rootRouter', ['$q', '$$grammar', '$$pipeline', function ($q, $$grammar, $$pipeline) {
/*
...
angular.module('ngComponentRouter').factory('$$grammar', ['$q', function ($q) {
/*
 * artisinal, handcrafted subset of the traceur runtime for picky webdevs
 */
